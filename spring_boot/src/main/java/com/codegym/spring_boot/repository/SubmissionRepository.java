...
public interface SubmissionRepository extends JpaRepository<Submission, Integer> {
    ...
    // Đếm số lần nộp bài chính thức (isTestRun=false) của user cho 1 problem trong contest
    int countByUserIdAndProblemIdAndContestIdAndIsTestRunFalse(Integer userId, Integer problemId,
                    Integer contestId);

    // Kiểm tra xem user đã có bài nộp nào cho problem này đạt trạng thái "AC" (100đ) chưa
    ...
    // Đếm số bài đã được nộp (distinct problemId, isTestRun=false) của user trong contest
