...
export const AdminProblemPage = () => {
    ...
    const [problemToDelete, setProblemToDelete] = useState<AdminProblemResponseDTO | null>(null);
    ...
    const handleDeleteConfirm = async () => {
        ...
        setIsDeleting(true);
        try {
            await adminProblemApi.deleteProblem(problemToDelete.id);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const problemsData = await adminProblemApi.getProblems();
    ...
    return (
        <AdminLayout title="Quản Lý Bài Tập" activeTab="problems">
            <div className="flex-1 overflow-y-auto p-8 bg-[#0f172a]">
                ...
                <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-4 mb-6">
                    ...
                    <div className="flex flex-wrap gap-4">
                        <div className="relative w-72">
                            ...
                            <input
                                type="text"
                                placeholder="Tìm ID, Tên, Người tạo..."
                                value={searchTerm}
                ...
                <div className="bg-[#1e293b]/70 backdrop-blur-md rounded-xl overflow-x-auto border border-slate-700/50">
                    <table className="w-full text-sm text-left text-slate-400 min-w-[900px]">
                        <thead className="text-xs uppercase bg-[#1e293b] text-slate-300">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-semibold text-center hidden md:table-cell">Danh Mục</th>
                        ...
                        <tbody>
                            ...
                            {!loading && !error && problems.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="text-center py-8 text-slate-400">Không có bài tập nào.</td>
                                </tr>
                            )}
                            {!loading && currentProblems.map((prob) => (
                                <tr key={prob.id} className="border-b border-[#1e293b] hover:bg-[#1e293b]/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 min-w-[150px]">
                                        {prob.authorUsername ? (
                                            <div>
                                                <div className="text-slate-200 font-semibold truncate" title={prob.authorName}>{prob.authorName}</div>
                                                <div className="text-xs text-slate-500 font-mono truncate" title={prob.authorUsername}>@{prob.authorUsername}</div>
                                            </div>
                                        ) : (
                                    ...
                                    <td className="px-6 py-4 text-center font-mono text-white hidden md:table-cell truncate max-w-[150px]">
                                        {prob.tags.map((tag) => tag.name).join(', ')}
                                    </td>
                                    <td className="px-6 py-4 text-right flex justify-end gap-2 whitespace-nowrap">
                                        <button
                                            onClick={() => navigate('/admin/testcases', { state: { problemId: prob.id } })}
                                        ...
                                        <button
                                            onClick={() => navigate(`/admin/problems/edit/${prob.id}`)}
